...
def cmd_add(args: list[str]) -> int:
    ...
    buy_date = args[4] if len(args) >= 5 else datetime.now(TAIPEI).strftime("%Y-%m-%d")
    cfg = load_positions()
    cfg.setdefault("open", []).append({
    ...
    save_positions(cfg)
    print(f"已加入：{name}（{sid}） {shares} 股 @ {price} ({buy_date})")
    return 0
...
def cmd_close(args: list[str]) -> int:
    if len(args) < 2:
        ...
        return 1
    sid, price = args[:2]
    sell_date = args[2] if len(args) >= 3 else datetime.now(TAIPEI).strftime("%Y-%m-%d")
    cfg = load_positions()
    open_list = cfg.get("open", [])
    matched = next((p for p in open_list if p["stock_id"] == sid), None)
    if matched is None:
        print(f"找不到持倉中的 {sid}")
        return 1
    open_list.remove(matched)
    pnl = calc_pnl_ntd(matched["buy_price"], float(price), matched["shares"])
    closed_entry = {**matched, "sell_date": sell_date, "sell_price": float(price), "pnl_ntd": pnl}
    cfg.setdefault("closed", []).append(closed_entry)
    save_positions(cfg)
    print(f"已平倉：{matched['stock_name']}（{sid}） @ {price}　已實現損益 {pnl:+,} 元")
    return 0
