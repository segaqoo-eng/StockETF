...
### 30 天歷史檢視 — **M**
資料層已就位（snapshots/），純前端工作。三種呈現粒度，先挑一個做：

| 方案 | 呈現 | 賣點 |
|---|---|---|
| A. **股票時光機** | 點股票 → 折線圖：每檔 ETF 對它的持股數 / 權重 30 天曲線 | 看「2330 被誰加碼/減碼」趨勢，FB 分享力強 |
| B. **ETF 變動表** | 點 ETF → 表格：每檔成分股 30 天 weight 每日一欄 | 一眼看出「這檔 ETF 哪些股票進進出出」 |
| C. **重大變動排行榜** | 首頁多一塊「過去 7 天加碼/砍最多 TOP 5」 | 像動態頁，打開就有重點，實作最小 |

建議先做 **C**（複用既有 `compute_diff` 邏輯擴大成 7-30 天累計）。
